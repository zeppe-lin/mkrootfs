...
packages.  This can be changed by C<-p>/C<--packages> command-line
option or by declaring C<ROOTFS_PKG> variable in
...
This command calls L<gpg(1)> with your default signing key.  It
doesn't need to be called via L<sudo(8)> or as C<root> unless the
...
create a tag in C<pkgsrc> repository (F</usr/src/pkgsrc>).
Example:
...
Secondly, go to Zeppe-Lin's C<pkgsrc> repository on the GitHub and
press C<Draft a new release>
(L<https://github.com/zeppe-lin/pkgsrc/release/new>).
